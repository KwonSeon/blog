  ...
  return (
    <>
      ...
      <p className="mt-4 text-base leading-8 text-muted-foreground">
        관리자 인증은 `username`, `password`를 same-origin Route Handler로 전송한 뒤,
        `HttpOnly cookie` 세션을 설정하고 보호 라우트로 이동하는 방식으로 구성합니다.
      </p>
      ...
      <form className="mt-8 grid gap-5" aria-label="관리자 로그인 form" onSubmit={handleSubmit}>
        ...
        <FormMessage>
          성공 시 브라우저 JS가 아닌 서버가 `HttpOnly cookie`를 설정하고, `next` query
          또는 기본 `/admin` 화면으로 이동합니다.
        </FormMessage>
